...
// returns a fiber handler that shall be used by producers in order to establish
// a webrtc connection. Moreover, it stores the in-comming streams in
// the server state and sends the local SDP to the client.
func Connect(appstate *state.State) func(*fiber.Ctx) error {
	return func(c *fiber.Ctx) error {
		defer utils.Recover(c)

		var body ApiConnectPayload
...
func Consume(appstate *state.State) func(*fiber.Ctx) error {
	return func(c *fiber.Ctx) error {
		...
		peer := appstate.Peers.Get(body.PeerId)
		if peer == nil {

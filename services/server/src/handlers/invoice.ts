...
import { isAdmin, isTutor, isTutorManager, isUser } from "@litespace/auth";
...
async function stats(req: Request, res: Response, next: NextFunction) {
  ...
  const user = req.user;
  const allowed =
    ((isTutor(user) || isTutorManager(user)) && user.id === tutorId) ||
    isAdmin(user);
...
function update(context: ApiContext) {
  return safeRequest(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      const itsTutor = isTutor(user) || isTutorManager(user);
      const allowed = isUser(user) && (itsTutor || isAdmin(user));
      if (!allowed) return next(forbidden());

...
async function create(req: Request, res: Response, next: NextFunction) {
  ...
  const list = await interviews.findOne({
...
async function find(req: Request, res: Response, next: NextFunction) {
  ...
  const result = await interviews.find({
    ...
    users: isTutor(user) ? [user.id] : users,
    interviewers,
    interviewees,
    interviewerFeedback,
    intervieweeFeedback,
  ...
  const response: IInterview.FindApiResponse = result;
...
async function update(req: Request, res: Response, next: NextFunction) {
  ...
  if (
    isRegularTutor(user) &&
    (intervieweeFeedback === undefined || status === undefined)
  )
    return next(empty());

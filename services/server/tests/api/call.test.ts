...
describe("/api/v1/call", () => {
  ...
  it("should retrieve call members by callId", async () => {
    ...
    // send request to the api
    const adminApi = await Api.forSuperAdmin();
    const res = await adminApi.atlas.call.findCallMembers(call.id)

    // test results
    expect(res.members).to.have.length(1);
    expect(res.members[0].callId).deep.equal(call.id);
    expect(res.members[0].userId).deep.equal(tutor.id);
  });

...
describe("/api/v1/user/", () => {
  ...
  describe("GET /api/v1/user/mystats", () => {
    beforeEach(async () => {
      await flush();
    });
    ...
    it("should retrieve student stats by current logged-in user id.", async () => {
      ...
      const res = await studentApi.atlas.user.findMyStats();
      
      expect(res.tutorCount).to.eq(3);
    ...
    it("should respond with forbidden if the user is not a student.", async () => {
      const tutorApi = await Api.forTutor();
      const res = await safe(async () => tutorApi.atlas.user.findMyStats());

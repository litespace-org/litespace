...
const Main: React.FC<{
  ...
  onLeave: Void;
}> = ({ self, member, start, duration, onLeave }) => {
    ...
    <div className="h-full">
      <PermissionsDialog
        onSubmit={async (permission) => {
           ...
           * duing the session)
           */
          if (!error && reRequestPermission) session.peer.addStream(result);
      ...
      {!session.peer.populatedTransceivers ? (
        <PreSession
          self={{
            id: self.id,
            image: self.image,
      ...
      {session.peer.populatedTransceivers && session.userMedia.stream ? (
        <Session

  ...
  return (
    <div
      className={cn(
        ...
        "bg-natural-50 md:w-[98px] lg:min-w-60 md:px-3 md:py-6 lg:p-6 md:shadow-app-sidebar",
        "flex md:justify-start md:flex-col md:gap-10",
        "md:shadow-mobile-sidebar"
      )}
    >
      <Header />
...
const MainPages: React.FC = () => {
  ...
  const pages: LinkInfo[] = useMemo(() => {
    ...
    if (user?.role === IUser.Role.Student && !md)
      return [tutors, lessonsSchedule, dashboard, chat, more];

...
import Session from "@/components/SessionV2";
import { asRateLessonQuery } from "@/lib/query";
import { router } from "@/lib/routes";
import { Web } from "@litespace/utils/routes";
import { useNavigate } from "react-router-dom";
import { EchoSocketProvider } from "@litespace/headless/echo";
...
const Content: React.FC<{ lessonId: number; self: IUser.Self }> = ({
  ...
  return (
    <EchoSocketProvider sessionId={lesson.data.lesson.sessionId}>
      <Session
        self={self}
        type="lesson"
        resourceId={lessonId}

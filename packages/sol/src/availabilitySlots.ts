...
export function canBook({ 
  ...
  const found = availableSubSlots.find(subslot => isSuperSlot(subslot, {
    parent: subslot.parent,
    start: dayjs.utc(bookInfo.start).toISOString(),
    end: dayjs.utc(bookInfo.start).add(bookInfo.duration, "minutes").toISOString(),
  }))
  return !!found;
}
...
export function isSuperSlot(
  ...
  return !dayjs.utc(a.start).isAfter(b.start) && !dayjs.utc(a.end).isBefore(b.end);
}

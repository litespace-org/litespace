...
describe("Time", () => {
    ...
    describe("Modify time", () => {
        ...
        it("should be able to set hours (midday)", () => {
            ...
            for (const [time, hours, modified] of tests) {
                expect(Time.from(time).setHours(hours, false).format("midday")).to.be.eq(modified);
            }
        });
        it("should be able to set minutes", () => {
            ...
            for (const [time, minutes, modified] of tests) {
                expect(Time.from(time).setMintues(minutes).format("midday")).to.be.eq(modified);
            }
        });
        it("should be able to set meridiem", () => {
            ...
            for (const [time, meridiem, modified] of tests) {
                expect(Time.from(time).setMeridiem(meridiem).format("midday")).to.be.eq(modified);
            }
        });
    });
    describe("Manipulate time", () => {
        it("should add/subtract mintues to the time", () => {
            ...
            for (const [input, minutes, output] of tests) {
                expect(Time.from(input).addMinutes(minutes).format("midday", null)).to.be.eq(output);
            }
        });
    ...
    describe("UTC and Local", () => {
        it("should convert local to utc time", () => {
            ...
            for (const [local, utc] of tests) {
                expect(Time.from(local).utc().format()).to.be.eq(Time.from(utc).format());
            }
        });
        it("should convert local time to utc", () => {
            ...
            for (const [local, utc] of tests) {
                expect(Time.from(local).local().format()).to.be.eq(Time.from(utc).format());
            }
        });
    });
});

...
export class Interviews {
  ...
  >): Promise<Paginated<IInterview.FullInterview>> {
    const baseBuilder = this.fullBuilder(tx);
    ...
    return { list: rows.map((row) => this.formatFull(row)), total };
  }
  ...
  formatFull(row: IInterview.PopulatedRow): IInterview.FullInterview {
    return {
      ids: {
        self: row.id,
        slot: row.slot_id,

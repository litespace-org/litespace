...
export class Tutors {
  ...
  async findTutorsByIntroVideos({
    ...
    count: number;
    order: "asc" | "desc";
    role: IUser.Role.Tutor | IUser.Role.TutorManager;
    activated?: boolean;
  }): Promise<number[]> {
    ...
    // NOTE: tutor-managers are only considered as intro-videos reviewers.
    const introVideoColumn =
      role === IUser.Role.TutorManager
        ? introVideos.column("reviewer_id")
        : introVideos.column("tutor_id");

    const rows = await filtered
      .leftJoin(introVideos.table, this.column("id"), introVideoColumn)
      .groupBy(this.column("id"))
      .count(introVideoColumn, { as: "introVideosCount" })

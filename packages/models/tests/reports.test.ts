...
describe("Reports", () => {
  ...
  describe(nameof(reports.create), () => {
    it("should successfully create a report", async () => {
      const report = await fixtures.report();
      expect(report).to.exist;
      expect(report.userId).to.exist;
      expect(report.title).to.exist;
      expect(report.description).to.exist;
  ...
  describe(nameof(reports.find), () => {
    ...
    it("should find reports for a speific user", async () => {
      ...
      expect(list).to.deep.eq(created.slice(0, 3));
    });
  });
});

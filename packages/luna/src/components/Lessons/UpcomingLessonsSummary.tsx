...
export const UpcomingLessonsSummary: React.FC<Props> = ({
  ...
  return (
    <div
      className={cn(
        "tw-border tw-border-natural-50 hover:tw-border-natural-100 tw-rounded-lg tw-p-6 tw-shadow-ls-small"
      )}
    >
      <Typography
        element="subtitle-2"
      ...
      <div>
        {lessons.length ? (
          <div className="tw-flex tw-flex-col tw-gap-4 tw-mb-[11px] tw-mt-4">
            {lessons.map((lesson) => (
              <Link
                to={lesson.url}

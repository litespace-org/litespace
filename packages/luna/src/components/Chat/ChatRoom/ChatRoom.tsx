  ...
  return (
    ...
    >
      <div className="tw-w-14 tw-h-14 tw-relative tw-rounded-full tw-shrink-0">
        ...
        {online && !isActive ? (
          <div className="tw-absolute tw-bottom-0 tw-right-1 tw-z-activityIndicator tw-w-3 tw-h-3 tw-rounded-full tw-bg-brand-500 tw-border-[.5px] tw-border-natural-50 " />
        ) : null}
      </div>
      <div className="tw-text-right tw-grow">
        ...
        >
          {name}
          {isMuted ? (
            <Muted className={cn(isActive && "[&>*]:tw-stroke-natural-50")} />
          ) : null}
        </Typography>
        <Typography
          tag="div"

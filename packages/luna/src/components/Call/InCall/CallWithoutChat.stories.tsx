...
/**
 * @description Common description: These are the stories without chat activated
 */

/**
 * @description Story: A user alone with camera enabled
 */
...
export const InternetProblem: StoryObj<Component> = {
  args: {
    chat: {
      enabled: false,
      toggle: () => {},
  ...
  render(props) {
    const FocusedStream = useCreateStream("focused", false);
    if (!FocusedStream) return <div></div>;
    return <Call {...props} streams={[FocusedStream]} internetProblem={true} />;
  },
};

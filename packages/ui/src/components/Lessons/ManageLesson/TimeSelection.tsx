...
export const TimeSelection: React.FC<{
  ...
  return (
    <div className="lg:tw-px-8">
      <div
        className={cn(
          "tw-grid tw-grid-cols-[repeat(auto-fill,minmax(60px,1fr))] tw-gap-2 md:tw-gap-4 tw-max-h-96 tw-overflow-y-auto",
          "tw-scrollbar-thin tw-scrollbar-thumb-neutral-200 tw-scrollbar-track-transparent tw-px-4 md:tw-px-6"
        )}
      >
        {slots.map((slot) => {
          return (
            <motion.button
              ...
              className={cn(
                ...
                "tw-mx-auto tw-h-12 tw-w-[59px] md:tw-h-[60px] md:tw-w-[60px] tw-rounded-lg tw-flex tw-items-center tw-justify-center",

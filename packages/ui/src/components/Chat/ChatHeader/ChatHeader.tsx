  ...
  return (
    <div
      className={cn(
        ...
        inSession
          ? "py-6 px-4 md:p-4 shadow-chat-header lg:shadow-transparent"
    ...
    >
      <div
        className={cn(
          ...
          inSession ? "gap-2 md:gap-6" : "gap-2"
        )}
      >
        <button
          type="button"
          onClick={back}
          className={"lg:hidden cursor-pointer w-6 h-6 md:w-8 md:h-8"}
        >
          <ArrowRightLong className="md:hidden" />
          <ArrowRight className="hidden md:block [&>*]:stroke-brand-700" />
        </button>
        <div
          className={cn(
            "flex gap-2 items-center",
            inSession ? "hidden md:flex md:gap-4 lg:gap-2" : "lg:gap-4"
          )}
        >
          <div
            className={cn(
